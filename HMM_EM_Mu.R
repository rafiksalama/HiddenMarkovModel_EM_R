...
a = rnorm(100,2,3)
b = rnorm(100,6,1)
data = c(a[1:15],b[1:20],a[15:30],b[40:100])
plot(data, type="l")
...
bs1 = dnorm(data,mean = s1m,s1s)*weightsA
bs2 = dnorm(data,mean = s2m,s2s)*weightsB
for(x in 1:3)
{
...
weightsA = (1/length(data)) * sum(gamma[,1])
weightsB = (1/length(data)) * sum(gamma[,2])
