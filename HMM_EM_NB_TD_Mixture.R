...
G = -0.47
X = 1:200
a = rpois(200,exp(B)*((X)^G))
b = rnbinom(80,size=1,mu=300)
data = c(a[1:60],b[1:20],a[61:100],b[21:40],a[101:160])
plot(data, type="l")
...
B1 = B2 = log(max(data))[1]
G1 = G2 = coef(lm(log(data)~log(X)))[2]

s12s = s22s = 2
s12m = s22m = mean(data)

bs1w1 = dpois(data,(exp(B1)*(X^G1)))+1e-300
bs1w2 = dnbinom(round(data),size = s12s , mu = s12m)+1e-300
bs2w1 = dpois(data,(exp(B2)*(X^G2)))+1e-300
...
gamma[,1] = log(alpha[,1])+log(beta[,1])
gamma[,2] = log(alpha[,2])+log(beta[,2])
...
G1 = sum(((log(data[-1])-B1)/log(X[-1]))*gammaM[-1,1])/sum(gammaM[-1,1])
...
colors = c("red","blue")
plot(data,type="b", col=colors[as.integer(apply(gammaM,1,function(x){(x[1]+x[3])>(x[2]+x[4])}))+1])
