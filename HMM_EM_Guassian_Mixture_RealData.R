...
Y = log1p(data)
b = solve(t(I)%*%I)%*%(t(I)%*%Y)
norm_data = log(data/(exp(b[1])*X^b[2]))
...
s11m = s12m = mean(norm_data)+sd(norm_data)
s21m = s22m = mean(norm_data)-sd(norm_data)
s11s = s12s = s21s = s22s = sd(norm_data)
...
bs1w1 = dnorm(norm_data,s11m,s11s)+1e-300
bs1w2 = dnorm(norm_data,s12m,s12s)+1e-300
bs2w1 = dnorm(norm_data,s21m,s21s)+1e-300
bs2w2 = dnorm(norm_data,s22m,s22s)+1e-300
...
s11m = sum(gammaM[,1]*norm_data)/sum(gammaM[,1])
s12m = sum(gammaM[,2]*norm_data)/sum(gammaM[,2])
s21m = sum(gammaM[,3]*norm_data)/sum(gammaM[,3])
s22m = sum(gammaM[,4]*norm_data)/sum(gammaM[,4])

s11s = sqrt(sum(gammaM[,1]*((norm_data-s11m)*t(norm_data-s11m)))/sum(gammaM[,1]))
s12s = sqrt(sum(gammaM[,2]*((norm_data-s12m)*t(norm_data-s12m)))/sum(gammaM[,2]))
s21s = sqrt(sum(gammaM[,3]*((norm_data-s21m)*t(norm_data-s21m)))/sum(gammaM[,3]))
s22s = sqrt(sum(gammaM[,4]*((norm_data-s22m)*t(norm_data-s22m)))/sum(gammaM[,4]))
...
colors = c("red","blue")
plot(norm_data,type="b", col=colors[as.integer(apply(gamma,1,function(x){(x[1])>(x[2])}))+1])
