...
l = (exp(B1))*((X)^B2)
beta = (l*0.2313)+1
a = rnbinom(length(X),mu = l,size=l/(beta-1))
b = rnbinom(20,size=1,mu=300)
data = c(a[1:30],rnbinom(20,size=1,mu=400),a[31:50],rnbinom(20,size=1,mu=400),a[51:70])
data = read.table("~/Documents/ResearchProjects/CC_Capture_Project/786O/CapC-786O/CapC-786O_REdig_sorted_CC1_chr8:134308799-134310949_NDRG1.gff")
data = data[data[,1]=="chr8",6]
max = which(data==max(data))
X = abs(c(1:length(data))-max)
range = c((max-500):(max+500))
X = X[range]
data = data[range]
plot(data, type="l")
...
l = (exp(B1))*((X)^B2)
beta = (l*0.2313)+1
bs1 = dnbinom(data,mu = l,size=l/(beta-1))
bs2 = dnbinom(data,mu = mu, size=size)
...
for(x in 1:100)
{
  ...
  xA = (X*gamma[,1])+1e-100
  yA = (data*gamma[,1])+1e-100
  xB = (X*gamma[,2])+1e-100
  yB = (data*gamma[,2])+1e-100
  Z = gamma[,1]>gamma[,2]
  x1 = 
  model = lm(log(yA)~log(xA))
  #B1 = coef(model)[1]
  #B2 = coef(model)[2]
  f = fitdistr(round(yA),"negative binomial")  
  size = f$estimate[1]
  mu = f$estimate[2]
